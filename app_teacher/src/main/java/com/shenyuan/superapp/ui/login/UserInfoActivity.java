 ...
 * desc 个人信息
 */
@Route(path = ARouterPath.AppTeacher.APP_USER_INFO)
public class UserInfoActivity extends BaseActivity<AcUserInfoBinding, UserPresenter> implements UserView {
    ...
    protected void addListener() {
        ...
        binding.tvLoginOut.setOnClickListener(v -> {
            new SimDialog.Builder(context).title(getString(R.string.logout_tip)).submitText(getString(R.string.loginout_sure)).onBacklistener(new BaseDialog.BaseOnBack() {
                @Override
        ...
        //修改手机号
        binding.clEditTel.setOnClickListener(v -> ARouter.getInstance().build(ARouterPath.AppTeacher.APP_EDIT_TEL)
                .navigation(this, modifyTelRequestCode));
    }
