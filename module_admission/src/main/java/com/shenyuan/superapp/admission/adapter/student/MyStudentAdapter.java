 ...
 * desc
 */
public class MyStudentAdapter extends BaseVBAdapter<StudentListBean, BaseDataBindingHolder> {

    private int moreState;

    public void setMoreState(int moreState) {
    ...
    protected void convert(@NotNull BaseDataBindingHolder holder, StudentListBean sm) {
        ...
        if (PermissionCommon.hasMyStudentReturn()) {
            bi.tvOption.setVisibility(View.VISIBLE);
        } else {
            bi.tvOption.setVisibility(View.GONE);
        }
        bi.tvOption.setText("退回");
        bi.tvOption.setPsBtnBackgroundColor(getValuesColor(R.color.color_f7d6d7));
        bi.tvOption.setTextColor(getValuesColor(R.color.color_ff5c5d));
