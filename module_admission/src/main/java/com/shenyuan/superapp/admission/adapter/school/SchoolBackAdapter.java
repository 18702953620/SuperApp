 ...
 * desc
 */
public class SchoolBackAdapter extends BaseVBAdapter<SchoolListBean, BaseDataBindingHolder> {
    public SchoolBackAdapter() {
        super(R.layout.item_dis_schools);
    }

    @SuppressLint("DefaultLocale")
    @Override
    protected void convert(@NotNull BaseDataBindingHolder baseViewHolder, SchoolListBean sm) {
        ItemDisSchoolsBinding bi = (ItemDisSchoolsBinding) baseViewHolder.getDataBinding();
        if (bi == null) {
            return;
        }
        bi.tvOption.setText(R.string.redistribution);
        //名称
        ...
        bi.tvSchoolAreaPerson.setText(String.format(getContext().getString(R.string.school_back_stafe), sm.getReturnedName()));
    }
}
