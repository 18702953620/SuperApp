...
public class SchoolTargetAdapter extends BaseVBAdapter<SchoolListBean, BaseDataBindingHolder> {
    ...
    public SchoolTargetAdapter() {
        ...
        hasDistrition = PermissionCommon.hasTargetSchoolReturn();
    }

    @SuppressLint("DefaultLocale")
    @Override
    protected void convert(@NotNull BaseDataBindingHolder holder, SchoolListBean sm) {
        ItemTargetSchoolsBinding bi = (ItemTargetSchoolsBinding) holder.getDataBinding();
        if (bi == null) {
            return;
        }
        //名称
        ...
        //地址
        bi.tvSchoolLocation.setText(String.format("%s%s%s", StrUtils.isEmpty(sm.getProvinceName()), StrUtils.isEmpty(sm.getCityName())
                , StrUtils.isEmpty(sm.getRegionName())));
        //所属区域
        bi.tvSchoolArea.setText(String.format("所属区域：%s", StrUtils.isEmpty(sm.getAreaName())));
        //区域负责人
        bi.tvSchoolAreaLeader.setText(String.format("区域负责人：%s", StrUtils.isEmpty(sm.getAreaStaffName())));

        //是否有权限
        if (hasDistrition) {
            bi.tvOption.setVisibility(View.VISIBLE);
        } else {
            bi.tvOption.setVisibility(View.GONE);
        }
        ...
        if (sm.getEnrollList() != null && sm.getEnrollList().size() > 0) {
            ...
            bi.tvSchoolTodayLabel.setText(String.format("%d年录取总数", dto.getYear()));
            ...
            if (sm.getEnrollList().size() > 1) {
                ...
                bi.tvSchoolYestodayLabel.setText(String.format("%d年录取总数", dt.getYear()));
            } else {
                bi.tvSchoolSpecialty.setText(String.valueOf(0));
                bi.tvSchoolYestodayLabel.setText(String.format("%d年录取总数", (dto.getYear() - 1)));
            }
        ...
        bi.tvSchoolAreaPerson.setText(String.format("宣传员：%s", sb.toString()));
    }
}
