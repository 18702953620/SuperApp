...
import com.shenyuan.superstudent.adapter.CustMoreAdapter;
...
import com.shenyuan.superstudent.bean.SortMenuBean;
import com.shenyuan.superstudent.databinding.AcMoreBinding;
import com.shenyuan.superstudent.databinding.ItemMoreBinding;
import com.shenyuan.superstudent.databinding.ItemMoreMenuBinding;
import com.shenyuan.superstudent.databinding.ItemMoreMenuLayoutBinding;
 ...
 * desc 更多
 */
@Route(path = ARouterPath.AppStudent.APP_STUDENT_MORE)
public class MoreActivity extends BaseStuActivity<AcMoreBinding, HomePresenter> implements HomeView {
    ...
    private HashMap<Integer, SortMenuBean> sortMenus = new HashMap<>();
        ...
        menuAdapter = new BaseQuickAdapter<ServiceBean, BaseDataBindingHolder>(R.layout.item_more_menu_layout) {
            ...
            protected void convert(@NotNull BaseDataBindingHolder baseDataBindingHolder, ServiceBean menu) {
                ...
                BaseQuickAdapter<ServiceBean.SysListDTO, BaseDataBindingHolder> serviceAdapter
                    ...
                    @Override
                    protected void convert(@NotNull BaseDataBindingHolder serviceHolder, ServiceBean.SysListDTO sm) {

                        ItemMoreMenuBinding layoutBinding = (ItemMoreMenuBinding) serviceHolder.getDataBinding();
                        ...
                        beanAdapter.setItemClickListener((bean, view, position) -> {
                            if (showEdit) {
                                sortMenus.put(bean.getId(), new SortMenuBean(baseDataBindingHolder.getAdapterPosition(),
                                        serviceHolder.getAdapterPosition(), position, bean));

                                custAdapter.addData(bean);

                                beanAdapter.remove(bean);
...
//                                ARouterPath.router(bean.getRouterUrl());
     ...
     * @param menuId menuId
     */
    private void resetMenuBean(int menuId) {
        if (sortMenus == null) {
            return;
        }
        SortMenuBean sortMenuBean = sortMenus.remove(menuId);
        if (sortMenuBean == null) {
            return;
        }

        ServiceBean serviceBean = menuAdapter.getData().get(sortMenuBean.getFristPosition());
        if (serviceBean != null && serviceBean.getSysList() != null) {
            ServiceBean.SysListDTO listDTO = serviceBean.getSysList().get(sortMenuBean.getParentPosition());
            if (listDTO != null && listDTO.getInfoList() != null) {
                listDTO.getInfoList().add(sortMenuBean.getPosition(), sortMenuBean.getMenuBean());
            }
        }
